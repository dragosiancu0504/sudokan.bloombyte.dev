# Terms and Conditions for Sudokan

**Last Updated: November 9, 2025**  
**Effective Date: November 9, 2025**

---

## Introduction

Welcome to **Sudokan**! These Terms and Conditions ("Terms", "Terms and Conditions") govern your use of the Sudokan mobile application (the "App", "Service") operated by **Bloom Byte SRL** ("we", "us", "our", "Company").

By downloading, installing, or using Sudokan, you agree to be bound by these Terms. If you do not agree to these Terms, please do not use the App.

**Important**: Please read these Terms carefully before using Sudokan. They contain important information about your rights, obligations, and limitations on our liability.

---

## Table of Contents

1. [Acceptance of Terms](#1-acceptance-of-terms)
2. [Eligibility](#2-eligibility)
3. [License Grant](#3-license-grant)
4. [Ownership and Intellectual Property](#4-ownership-and-intellectual-property)
5. [User Conduct and Prohibited Activities](#5-user-conduct-and-prohibited-activities)
6. [Privacy and Data Collection](#6-privacy-and-data-collection)
7. [In-App Advertising](#7-in-app-advertising)
8. [Rewarded Ads](#8-rewarded-ads)
9. [Third-Party Services](#9-third-party-services)
10. [App Updates and Changes](#10-app-updates-and-changes)
11. [Disclaimer of Warranties](#11-disclaimer-of-warranties)
12. [Limitation of Liability](#12-limitation-of-liability)
13. [Indemnification](#13-indemnification)
14. [Termination](#14-termination)
15. [Modifications to Terms](#15-modifications-to-terms)
16. [Governing Law and Jurisdiction](#16-governing-law-and-jurisdiction)
17. [Dispute Resolution](#17-dispute-resolution)
18. [EU Consumer Rights](#18-eu-consumer-rights)
19. [California Consumer Rights](#19-california-consumer-rights)
20. [Severability](#20-severability)
21. [Waiver](#21-waiver)
22. [Entire Agreement](#22-entire-agreement)
23. [Contact Information](#23-contact-information)

---

## 1. Acceptance of Terms

### 1.1 Binding Agreement

By accessing or using Sudokan, you agree to:
- ✅ Be bound by these Terms and Conditions
- ✅ Comply with all applicable laws and regulations
- ✅ Accept our Privacy Policy (separate document, available in-app)

**Your acceptance** is indicated by:
- Downloading the App from the Apple App Store or Google Play Store
- Installing the App on your device
- Creating or continuing a game
- Using any feature of the App

### 1.2 Age Requirement

You must be at least **13 years of age** to use Sudokan. If you are under 13, you may not use the App under any circumstances.

If you are between 13 and the age of majority in your jurisdiction (typically 18), you represent that:
- Your parent or legal guardian has reviewed and agreed to these Terms on your behalf
- You have permission to use the App

**Parents/Guardians**: If your child is using Sudokan, you are responsible for their use of the App and compliance with these Terms.

### 1.3 Amendments to Terms

We reserve the right to modify these Terms at any time (see Section 15 for details). Your continued use of the App after changes constitutes acceptance of the revised Terms.

### 1.4 Additional Terms

Some features of Sudokan may be subject to additional terms or policies:
- **Privacy Policy** (how we collect and use your data)
- **Third-party licenses** (open-source software used in the App)
- **App Store terms** (Apple App Store Terms of Service, Google Play Terms of Service)

In case of conflict, these Terms prevail, unless specific terms expressly override them.

---

## 2. Eligibility

### 2.1 Age Restrictions

**Minimum Age**: 13 years old

**Why**: 
- Compliance with COPPA (Children's Online Privacy Protection Act - United States)
- GDPR requirements for children's data processing (European Union)
- Advertising content may not be appropriate for younger children

**Enforcement**:
- We do NOT verify ages (no account creation required)
- We rely on app store age ratings and this provision
- If we discover a user is under 13, we may terminate access

### 2.2 Legal Capacity

By using Sudokan, you represent and warrant that:
- ✅ You have the legal capacity to enter into binding contracts
- ✅ You are not barred from using the App under applicable laws
- ✅ You will use the App in compliance with these Terms and all laws

### 2.3 Geographic Restrictions

Sudokan is available worldwide via the Apple App Store and Google Play Store, subject to:
- App store availability in your country
- Compliance with local laws and regulations
- Export control laws (no use in embargoed countries)

**Restricted Countries**: We do not knowingly permit use in countries under comprehensive U.S. sanctions (e.g., North Korea, Syria, Iran, Cuba - subject to change based on current sanctions).

If you access the App from a prohibited location, you are in breach of these Terms.

---

## 3. License Grant

### 3.1 Limited, Non-Exclusive License

Subject to your compliance with these Terms, Bloom Byte SRL grants you a **limited, non-exclusive, non-transferable, non-sublicensable, revocable license** to:

- ✅ Download and install Sudokan on devices you own or control
- ✅ Use the App for your personal, non-commercial entertainment purposes
- ✅ Access features and content available in the App

**This license does NOT include the right to**:
- ❌ Modify, translate, reverse engineer, or create derivative works
- ❌ Distribute, sell, rent, lease, or sublicense the App
- ❌ Use the App for commercial purposes (e.g., selling puzzle solutions, creating competing apps)
- ❌ Remove or alter copyright notices, trademarks, or other proprietary markings
- ❌ Access the App using unauthorized tools or software (e.g., bots, scrapers)

### 3.2 App Store Terms

Your use of Sudokan is also subject to the app store's terms:

**Apple App Store**:
- [Apple Media Services Terms and Conditions](https://www.apple.com/legal/internet-services/itunes/us/terms.html)
- You acknowledge that Apple is not responsible for the App or its content
- Apple has no obligation to provide support or maintenance

**Google Play Store**:
- [Google Play Terms of Service](https://play.google.com/about/play-terms/)
- You acknowledge that Google is not responsible for the App or its content
- Google has no obligation to provide support or maintenance

**In case of conflict**: App store terms apply for issues related to the store (e.g., refunds for paid apps - N/A for Sudokan since it's free). These Terms apply for issues related to the App itself.

### 3.3 Multiple Devices

You may install Sudokan on multiple devices you own or control (e.g., your iPhone and iPad), but:
- Game progress is NOT synced across devices (stored locally)
- Each device is a separate installation
- You cannot share installations with other users

### 3.4 Revocation of License

We may revoke your license to use Sudokan at any time if:
- You breach these Terms
- We are required to do so by law
- We discontinue the App (see Section 10)

Upon revocation, you must immediately:
- Stop using the App
- Uninstall the App from all devices
- Destroy all copies in your possession

---

## 4. Ownership and Intellectual Property

### 4.1 Our Intellectual Property

**Sudokan and all its components are owned by Bloom Byte SRL**. This includes, but is not limited to:

**Copyright** (protected under international copyright law):
- ✅ App code (software, algorithms, backend logic)
- ✅ User interface design (screens, layouts, graphics)
- ✅ Puzzle generation algorithms (our proprietary Sudoku generator)
- ✅ Text and content (tutorial text, instructions, help documentation)
- ✅ Audio (sound effects, if any)
- ✅ Graphics and visual assets (icons, illustrations, color schemes)

**Trademarks** (owned by Bloom Byte SRL):
- ✅ "Sudokan" name
- ✅ Sudokan logo and app icon
- ✅ "Bloom Byte" name and logo

**Trade Secrets**:
- ✅ Puzzle generation algorithms
- ✅ Dynamic Difficulty Adjustment (DDA) logic
- ✅ Ranking system scoring formulas
- ✅ Business strategies and analytics

**Patents**: Currently, we have no patents related to Sudokan. If we obtain patents in the future, they will be noted here.

### 4.2 Third-Party Intellectual Property

Sudokan uses third-party software and services:

**Open-Source Software**:
- Flutter framework (BSD License)
- Dart language (BSD License)
- Firebase SDK (Apache License 2.0)
- Hive database (Apache License 2.0)
- Provider (MIT License)
- Google Fonts (Open Font License)
- Other dependencies (see app's "Licenses" screen)

**Acknowledgment**: We acknowledge and comply with all open-source licenses. Full license texts are available in the App (Settings → About → Licenses).

**Third-Party Services**:
- Google Firebase (Analytics, Crashlytics, Firestore)
- Google AdMob (advertising)
- Ad network partners (see Privacy Policy)

These services are subject to their own terms and intellectual property rights.

### 4.3 User Content (Minimal)

**What you create in Sudokan**:
- Puzzle solutions (numbers you enter)
- Game progress and statistics
- Feedback submissions (if you provide feedback)

**Your ownership**: You retain ownership of your puzzle solutions and feedback.

**Our license**: By submitting feedback, you grant us a **perpetual, irrevocable, worldwide, royalty-free, non-exclusive license** to:
- Use, reproduce, modify, and display your feedback
- Improve the App based on your suggestions
- Incorporate your ideas into future versions (without attribution or compensation)

**No obligation**: We have no obligation to use your feedback or compensate you for it.

**Privacy**: User-generated content (feedback) is subject to our Privacy Policy.

### 4.4 Prohibited Uses of Our Intellectual Property

You may NOT:
- ❌ Copy, reproduce, or distribute the App or its content
- ❌ Reverse engineer, decompile, or disassemble the App
- ❌ Extract our puzzle generation algorithms
- ❌ Use the "Sudokan" name or logo without written permission
- ❌ Create derivative works (e.g., a competing Sudoku app using our code)
- ❌ Frame, mirror, or scrape content from the App
- ❌ Use automated tools (bots, scrapers) to access the App

**Penalties**: Violation may result in:
- Termination of your license (Section 14)
- Legal action for copyright/trademark infringement
- Damages and attorney fees (if we prevail in court)

### 4.5 DMCA and Copyright Infringement

**If you believe your intellectual property has been infringed** by content in Sudokan:

**Submit a DMCA Notice to**:
- **Email**: contact@bloombyte.dev
- **Subject**: "DMCA Takedown Request"
- **Include**:
  1. Description of the copyrighted work
  2. Location of infringing material in the App
  3. Your contact information
  4. Statement of good faith belief
  5. Statement under penalty of perjury that information is accurate
  6. Your physical or electronic signature

**Response**: We'll investigate and respond within 7-14 days.

**Counter-Notice**: If we remove content based on a DMCA notice, you may submit a counter-notice if you believe the removal was erroneous.

---

## 5. User Conduct and Prohibited Activities

### 5.1 Acceptable Use

Sudokan is for **personal, non-commercial entertainment use only**. You agree to:

- ✅ Use the App in accordance with these Terms
- ✅ Comply with all applicable laws
- ✅ Respect our intellectual property rights
- ✅ Not interfere with other users (though Sudokan is single-player)

### 5.2 Prohibited Activities

You agree NOT to:

**Technical Abuse**:
- ❌ Reverse engineer, decompile, or disassemble the App
- ❌ Use bots, scripts, or automated tools to interact with the App
- ❌ Attempt to hack, crack, or exploit vulnerabilities
- ❌ Access or attempt to access our servers or backend systems
- ❌ Interfere with the App's functionality or security measures
- ❌ Introduce viruses, malware, or harmful code

**Intellectual Property Violations**:
- ❌ Copy, redistribute, or create derivative works
- ❌ Extract puzzle generation algorithms or proprietary code
- ❌ Use our trademarks without permission

**Commercial Exploitation**:
- ❌ Sell, rent, lease, or sublicense the App
- ❌ Use the App for commercial purposes (e.g., tournaments with entry fees)
- ❌ Advertise or promote products/services through the App (no user-generated ads)

**Fraud and Abuse**:
- ❌ Provide false information (e.g., in feedback submissions)
- ❌ Impersonate Bloom Byte SRL or our representatives
- ❌ Attempt to manipulate ad impressions or rewards (ad fraud)

**Legal Violations**:
- ❌ Use the App for illegal activities
- ❌ Violate export control laws or sanctions
- ❌ Violate third-party rights (privacy, publicity, intellectual property)

### 5.3 Ad Fraud and Abuse

**Rewarded ads** (watch ads to earn hints/mistake limit) are subject to fraud detection:

**Prohibited behaviors**:
- ❌ Using automated tools to generate ad impressions
- ❌ Creating multiple accounts/installations to farm rewards
- ❌ Clicking ads without genuine interest (click fraud)
- ❌ Manipulating device settings to exploit ad system

**Penalties**:
- Termination of license
- Reporting to ad network partners (may result in device bans)
- Legal action for fraud (in extreme cases)

**How we detect**: Google AdMob and Firebase Analytics detect unusual patterns (e.g., excessive ad watches, abnormal click-through rates).

### 5.4 Consequences of Violations

If you violate these Terms, we may:
1. **Warn you** (first-time minor violations)
2. **Terminate your license** (immediate uninstall required)
3. **Report to authorities** (for illegal activities)
4. **Seek legal remedies** (damages, injunctions, attorney fees)

**No refunds**: If we terminate your license, you are not entitled to refunds (app is free, but principle applies to any future paid features).

### 5.5 Reporting Violations

**If you discover someone violating these Terms or exploiting the App**:
- **Email**: contact@bloombyte.dev
- **Subject**: "Terms Violation Report"
- **Include**: Description of violation, evidence (screenshots), device/account details (if applicable)

We appreciate responsible reporting and will investigate promptly.

---

## 6. Privacy and Data Collection

### 6.1 Privacy Policy Governs Data Collection

**Your use of Sudokan is subject to our Privacy Policy**, which explains:
- What data we collect (analytics, crash reports, ads, feedback)
- How we use your data (app improvement, bug fixing, advertising)
- Your rights (access, deletion, correction, opt-out)
- Third-party data sharing (Google Firebase, AdMob, ad networks)

**Privacy Policy Location**:
- In-app: Settings → Privacy Policy
- Online: bloombyte.dev/privacy (if published)

**By using Sudokan, you acknowledge that you have read and understood the Privacy Policy.**

### 6.2 Consent-Based Data Collection

Most data collection is **optional and requires your consent**:
- ✅ **Analytics** (opt-in via Privacy Consent screen)
- ✅ **Crash reporting** (opt-in via Privacy Consent screen)
- ✅ **Personalized ads** (opt-in via Privacy Consent screen)
- ✅ **Feedback** (voluntary submission)

**You can withdraw consent anytime** in Privacy Settings.

### 6.3 Data Retention

- **Local data** (game progress): Stored on your device until you uninstall
- **Analytics**: 60 days (auto-deleted by Firebase)
- **Crash reports**: 90 days (auto-deleted)
- **Feedback**: 12 months (auto-deleted)

See Privacy Policy for full details.

### 6.4 Children's Privacy

**Sudokan is not directed at children under 13**. We comply with:
- COPPA (US)
- GDPR-K (EU children's privacy)

**Parents**: If your child under 13 uses Sudokan, please:
- Disable analytics, crash reporting, and personalized ads in Privacy Settings
- Supervise their use

See Privacy Policy Section 8 for full children's privacy details.

### 6.5 Your Privacy Rights

Depending on your location, you have rights regarding your data:
- **GDPR** (EU): Access, erasure, correction, portability, object, complaint
- **CCPA/CPRA** (California): Know, delete, opt-out, correct, non-discrimination
- **LGPD** (Brazil): Access, correction, deletion, portability

**To exercise rights**: Email contact@bloombyte.dev (see Privacy Policy Section 7 for full instructions).

---

## 7. In-App Advertising

### 7.1 Ad-Supported Model

Sudokan is **free to use** and supported by advertising. By using the App, you acknowledge that:
- ✅ Ads will be displayed during gameplay
- ✅ Ads are served by Google AdMob and third-party ad networks
- ✅ Ad frequency and placement are at our discretion
- ✅ You may see duplicate ads or ads you've seen before

**No ad-free version**: Currently, we do not offer a paid, ad-free version of Sudokan. If we do in the future, these Terms will be updated.

### 7.2 Types of Ads

**Non-Personalized Ads** (default):
- Shown to all users who don't consent to personalized ads
- Based on contextual information (you're playing a puzzle game)
- No cross-app tracking or interest-based targeting

**Personalized Ads** (if you consent):
- Targeted based on your interests and behavior
- Uses Advertising ID (IDFA/AAID) for cross-app tracking
- May show ads for products/services you've shown interest in elsewhere
- Subject to 100+ ad network partners' policies (see Privacy Policy)

**Your Control**: Disable personalized ads in Privacy Settings → "Personalized Ads" toggle.

### 7.3 Ad Content Disclaimer

**We do NOT control which specific ads are shown**. Ads are served by Google AdMob and ad networks.

**What we aim for**:
- Family-friendly content (no explicit/adult ads)
- Relevant to puzzle/gaming audience
- Compliant with app store policies

**However**:
- ❌ We cannot guarantee ad content suitability 100% of the time
- ❌ Ad networks may occasionally serve inappropriate ads (report to us)
- ❌ Ads may link to third-party websites/apps (not under our control)

**If you see an inappropriate ad**:
- **Report to us**: contact@bloombyte.dev (include screenshot, description)
- **We'll report to AdMob**: Google will investigate and block the ad

### 7.4 Ad Interactions

**Clicking on ads**:
- May open a third-party website or app store listing
- You leave Sudokan and enter a third-party service
- **We're not responsible** for third-party content, privacy practices, or purchases
- Review third-party terms and privacy policies before providing information

**Accidental clicks**:
- Mobile ads are small and clicks may be accidental
- You can close most ads immediately after they open
- If you accidentally install an app, you can delete it

**Ad fraud**: Do NOT click ads with the sole intent of generating revenue for us (click fraud). AdMob detects and penalizes this behavior.

### 7.5 Ad Network Partners

Google AdMob works with **100+ third-party ad networks**, including:
- Amazon Publisher Services
- AppLovin
- Chartboost
- InMobi
- ironSource
- Unity Ads
- Vungle
- Meta Audience Network
- TikTok For Business

**Full list**: [Google's Authorized Buyers](https://support.google.com/admob/answer/9012903)

**Privacy**: Each ad network has its own privacy policy. See our Privacy Policy Section 4.2 for details.

### 7.6 Ad Blockers

**Use of ad blockers** (e.g., DNS-based ad blocking, VPNs with ad blocking):
- May prevent ads from displaying
- **Not a violation of these Terms** (we don't prohibit it)
- However, ad revenue supports free access to Sudokan
- We appreciate users who allow ads to support continued development

---

## 8. Rewarded Ads

### 8.1 How Rewarded Ads Work

Sudokan offers **optional rewarded ads** that provide in-game benefits:

**Rewards**:
- **Extra hints**: Watch an ad to earn 3 additional hints
- **Mistake limit increase**: Watch an ad to increase your mistake limit

**Voluntary**: You are NEVER required to watch rewarded ads. They are entirely optional.

### 8.2 Reward Fulfillment

**If you watch a rewarded ad**:
- ✅ You must watch the full ad (typically 15-30 seconds)
- ✅ Reward is granted immediately upon completion
- ✅ Reward is applied to your current game session

**If the ad fails to load or is skipped**:
- ❌ No reward is granted
- ❌ We're not responsible for ad loading failures (network issues, ad availability)
- ❌ You can try again (no limit on attempts, subject to ad availability)

### 8.3 No Guarantee of Ad Availability

**Ad availability depends on**:
- Your location (ad networks may not serve ads in all countries)
- Time of day (ad demand fluctuates)
- Ad network inventory
- Your device settings (if you block ads)

**We do NOT guarantee**:
- ❌ Rewarded ads will always be available
- ❌ A specific number of rewarded ads per day
- ❌ Any particular reward type at any given time

**If ads are unavailable**: You can still play without rewards. Hints and mistake limits are part of the game's challenge.

### 8.4 Abuse Prevention

**Abuse of rewarded ads** (e.g., using bots to farm rewards) is prohibited (see Section 5.3).

**Detection**:
- Google AdMob tracks ad impression patterns
- Firebase Analytics detects abnormal reward usage
- Unusual activity may trigger investigation

**Penalties**: Termination of license, reporting to ad networks.

### 8.5 No Monetary Value

**Rewards have NO monetary value**:
- ❌ Cannot be exchanged for cash
- ❌ Cannot be transferred to other users
- ❌ Cannot be refunded
- ❌ Lost upon uninstall (rewards are not backed up)

**Rewards are virtual items** with no real-world value, provided as a convenience for gameplay.

---

## 9. Third-Party Services

### 9.1 Third-Party Service Providers

Sudokan integrates with third-party services to provide functionality:

| Service | Provider | Purpose | Terms Link |
|---------|----------|---------|------------|
| **Firebase Analytics** | Google LLC | App usage analytics | [Firebase Terms](https://firebase.google.com/terms) |
| **Firebase Crashlytics** | Google LLC | Crash reporting | [Firebase Terms](https://firebase.google.com/terms) |
| **Google AdMob** | Google LLC | In-app advertising | [AdMob Terms](https://support.google.com/admob/answer/7356219) |
| **Cloud Firestore** | Google LLC | Feedback storage | [Firebase Terms](https://firebase.google.com/terms) |
| **Daily Challenge API** | Bloom Byte SRL | Daily puzzle delivery | (Our own server, no separate terms) |

**Your use of these services** is subject to their respective terms and privacy policies.

### 9.2 No Endorsement

**Links to third-party websites** (e.g., in ads, privacy policy links):
- We do NOT endorse or control third-party sites/apps
- We're not responsible for third-party content, privacy, or practices
- Review third-party terms before using their services

**Third-party ads**: Ads displayed in Sudokan are not endorsements. We don't vouch for advertised products/services.

### 9.3 Third-Party Liability

**We're not responsible for**:
- ❌ Third-party service failures (e.g., Firebase downtime)
- ❌ Third-party data breaches or security incidents
- ❌ Third-party privacy violations
- ❌ Products/services advertised in ads
- ❌ Third-party websites linked from the App

**If you have issues with a third party**: Contact them directly. We'll assist where we can, but we cannot resolve third-party disputes.

### 9.4 Changes to Third-Party Services

We may:
- ✅ Add, remove, or replace third-party services at any time
- ✅ Update this section to reflect changes
- ✅ Notify you via in-app announcement (for material changes)

**No guarantee**: We don't guarantee any specific third-party service will always be available.

---

## 10. App Updates and Changes

### 10.1 Updates and New Versions

We may release updates to Sudokan to:
- Fix bugs and improve stability
- Add new features
- Update puzzle generation algorithms
- Improve user interface
- Comply with legal requirements
- Enhance security

**Your responsibility**:
- ✅ Keep Sudokan updated to the latest version
- ✅ Download updates from official app stores only (Apple App Store, Google Play)

**Update frequency**: Updates may be released as needed (no fixed schedule).

### 10.2 Automatic Updates

**If you enable automatic updates** (device setting):
- The App will update automatically when new versions are available
- You acknowledge updates may change features or Terms

**If you disable automatic updates**:
- You must manually update
- Old versions may not be supported (see Section 10.5)

### 10.3 Changes to Features

We may:
- ✅ Add new features (e.g., new difficulty levels, game modes)
- ✅ Modify existing features (e.g., change UI design, adjust DDA algorithm)
- ✅ Remove features (e.g., discontinue Daily Challenge)

**No guarantee**: We don't guarantee any specific feature will be available indefinitely.

**Your recourse**: If you dislike changes, you may stop using the App (see Section 14).

### 10.4 Beta Versions and Early Access

**If we offer beta versions** (e.g., via TestFlight, Google Play Beta):
- Beta versions may be unstable and contain bugs
- You use beta versions at your own risk
- Beta versions may have separate terms

**No obligation**: We're not obligated to release features tested in beta to the public.

### 10.5 End of Support for Old Versions

**We may discontinue support for old app versions**:
- Old versions may not receive bug fixes or security updates
- Old versions may not work with our backend services (e.g., Daily Challenge API)
- We'll provide notice when support ends (typically 6-12 months after new version release)

**Your responsibility**: Update to the latest version to maintain full functionality.

### 10.6 Discontinuation of Sudokan

**We reserve the right to discontinue Sudokan** at any time, for any reason, including:
- Business reasons (e.g., not financially viable)
- Technical reasons (e.g., unsustainable server costs)
- Legal reasons (e.g., regulatory changes)

**If we discontinue Sudokan**:
- ✅ We'll provide at least **30 days' notice** (in-app announcement and website)
- ✅ The App will be removed from app stores (no new downloads)
- ✅ Existing installations may continue to work (for local-only features)
- ✅ Cloud-based features (Daily Challenge, feedback) will cease to function
- ❌ No refunds (app is free)

**Your local data** (game progress, settings) will remain on your device until you uninstall.

**No ongoing support**: After discontinuation, we have no obligation to provide updates, support, or maintain servers.

---

## 11. Disclaimer of Warranties

### 11.1 "AS IS" and "AS AVAILABLE" Basis

**SUDOKAN IS PROVIDED ON AN "AS IS" AND "AS AVAILABLE" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED.**

To the maximum extent permitted by law, Bloom Byte SRL disclaims all warranties, including but not limited to:

**Implied Warranties**:
- ❌ **Merchantability**: No warranty that the App is suitable for your purposes
- ❌ **Fitness for a particular purpose**: No warranty the App meets your specific needs
- ❌ **Non-infringement**: No warranty the App doesn't infringe third-party rights
- ❌ **Title**: No warranty of clear title to the App

**Operational Warranties**:
- ❌ **Uninterrupted service**: The App may experience downtime, bugs, or crashes
- ❌ **Error-free operation**: The App may contain bugs, glitches, or errors
- ❌ **Security**: No guarantee the App is free from viruses or security vulnerabilities
- ❌ **Compatibility**: No guarantee the App works on all devices or OS versions

**Content Warranties**:
- ❌ **Accuracy**: Puzzle solutions may have errors (though we strive for accuracy)
- ❌ **Completeness**: Features may be incomplete or experimental
- ❌ **Timeliness**: Daily Challenge may not update on time due to server issues

### 11.2 No Guarantee of Availability

We do NOT guarantee that:
- ❌ Sudokan will always be available (server downtime may occur)
- ❌ Features will work as expected 100% of the time
- ❌ The App will be compatible with future OS updates
- ❌ Ads will always load (depends on third-party ad networks)
- ❌ Daily Challenge will always be accessible (server dependency)

**Your acceptance**: By using Sudokan, you acknowledge these limitations.

### 11.3 Third-Party Services Disclaimer

**We're not responsible for**:
- ❌ Google Firebase/AdMob service failures
- ❌ Ad network failures or inappropriate ad content
- ❌ Third-party websites linked from ads
- ❌ App store (Apple/Google) policies or availability

**Third-party services** are subject to their own terms and warranties (or lack thereof).

### 11.4 User-Generated Content Disclaimer

**Feedback submissions**:
- We don't warrant accuracy, usefulness, or non-infringement of user feedback
- We're not liable for feedback content (you're responsible for what you submit)

### 11.5 No Professional Advice

**Sudokan is an entertainment app**, not:
- ❌ Educational software (though it may improve cognitive skills)
- ❌ Medical or therapeutic device (not for treating health conditions)
- ❌ Professional training tool

**Do not rely on Sudokan for professional purposes.**

### 11.6 Jurisdiction-Specific Warranty Rights

**Some jurisdictions do not allow disclaimer of implied warranties**. If this applies to you:
- The disclaimers above may not apply in full
- You may have additional rights under consumer protection laws
- See Sections 18 (EU Consumer Rights) and 19 (California Consumer Rights)

**To the extent disclaimers are not permitted**, they are limited to the maximum extent allowed by law.

---

## 12. Limitation of Liability

### 12.1 No Liability for Damages

**TO THE MAXIMUM EXTENT PERMITTED BY LAW, BLOOM BYTE SRL, ITS OFFICERS, DIRECTORS, EMPLOYEES, AND AFFILIATES SHALL NOT BE LIABLE FOR ANY DAMAGES ARISING FROM YOUR USE OF SUDOKAN, INCLUDING BUT NOT LIMITED TO:**

**Direct Damages**:
- ❌ Loss of data (game progress, statistics)
- ❌ Device damage (e.g., battery drain, overheating - though highly unlikely)
- ❌ Time wasted (playing the game or dealing with bugs)

**Indirect, Incidental, Consequential, or Special Damages**:
- ❌ Lost profits or revenue
- ❌ Loss of business opportunities
- ❌ Loss of goodwill or reputation
- ❌ Emotional distress or frustration
- ❌ Costs of substitute services

**Punitive or Exemplary Damages**:
- ❌ Damages intended to punish us (even if we're negligent)

**Third-Party Damages**:
- ❌ Damages caused by Google Firebase, AdMob, or ad networks
- ❌ Damages from third-party websites/apps accessed via ads
- ❌ Damages from app store policies (Apple/Google)

### 12.2 Theories of Liability

**This limitation applies regardless of the legal theory**, including:
- Breach of contract
- Breach of warranty
- Negligence
- Strict liability
- Tort
- Any other legal theory

**Even if we've been advised** of the possibility of such damages.

### 12.3 Maximum Liability Cap

**IF WE ARE FOUND LIABLE** (despite the above limitations), our total liability shall NOT exceed:

**$100 USD (One Hundred U.S. Dollars)** or the amount you paid to use Sudokan in the 12 months preceding the claim, **whichever is less**.

**Since Sudokan is free**, our maximum liability is effectively **$100 USD**.

This cap applies to:
- All claims combined (not per claim)
- All theories of liability
- All damages (direct, indirect, consequential, etc.)

### 12.4 Basis of the Bargain

**These limitations are fundamental to our agreement**. We offer Sudokan for free, and these limitations reflect that:

- The App is free (no payment, no revenue from users directly)
- We cannot afford unlimited liability for a free app
- You accept these risks in exchange for free access

**Without these limitations**, we could not offer Sudokan on a free basis.

### 12.5 Jurisdiction-Specific Liability Limits

**Some jurisdictions do not allow limitation of liability for**:
- Personal injury
- Fraudulent misrepresentation
- Gross negligence or willful misconduct
- Certain consumer protection violations

**If this applies to you**:
- The limitations above may not apply in full
- We remain liable to the extent required by law
- See Sections 18 (EU Consumer Rights) and 19 (California Consumer Rights)

**To the extent liability cannot be limited**, it is limited to the maximum extent allowed by law.

### 12.6 Allocation of Risk

**You acknowledge and agree that**:
- ✅ You use Sudokan at your own risk
- ✅ You're responsible for backing up important data (though Sudokan has no personal data)
- ✅ You're responsible for device security (screen lock, OS updates)
- ✅ You understand the App may have bugs or errors

**Fair allocation**: These Terms allocate risks between you and us. You accept certain risks in exchange for free access to the App.

---

## 13. Indemnification

### 13.1 Your Indemnification Obligation

**You agree to indemnify, defend, and hold harmless** Bloom Byte SRL, its officers, directors, employees, agents, affiliates, and service providers from and against any and all claims, damages, losses, liabilities, costs, and expenses (including reasonable attorney fees) arising from or relating to:

**Your Use of the App**:
- ✅ Your violation of these Terms
- ✅ Your violation of any law or regulation
- ✅ Your violation of third-party rights (intellectual property, privacy, publicity)

**Your Content**:
- ✅ Feedback submissions that infringe third-party rights
- ✅ False or misleading information you provide

**Your Conduct**:
- ✅ Prohibited activities (Section 5)
- ✅ Reverse engineering or hacking attempts
- ✅ Ad fraud or abuse

**Third-Party Claims**:
- ✅ Claims by other users or third parties related to your use of Sudokan

### 13.2 What Indemnification Means

**If someone sues us because of your actions**, you agree to:
1. **Defend us**: Hire attorneys and represent us in court (at your expense)
2. **Pay damages**: Cover any judgment, settlement, or award against us
3. **Pay costs**: Cover attorney fees, court costs, and related expenses

**Example**: If you reverse engineer Sudokan and sell a competing app using our code, and we're sued for copyright infringement by a third party (or vice versa), you indemnify us.

### 13.3 Conditions for Indemnification

**We'll notify you** of any claim subject to indemnification.

**You'll cooperate** by providing information, documents, and testimony as needed.

**We reserve the right** to:
- Participate in defense (at our expense)
- Approve settlement terms (we won't unreasonably withhold approval)

### 13.4 Exceptions

**You are NOT required to indemnify us for**:
- ❌ Our own negligence or willful misconduct
- ❌ Defects in the App caused by our design or code
- ❌ Claims solely arising from our violation of law

**Mutual responsibility**: If a claim arises from both your conduct and our fault, we'll allocate responsibility fairly.

---

## 14. Termination

### 14.1 Termination by You

**You may stop using Sudokan at any time** by:
- Uninstalling the App from all devices
- Ceasing to access the App

**No notice required**: You don't need to notify us.

**Effect of termination**:
- Your license to use the App ends immediately (Section 3)
- Local data (game progress) remains on your device until you delete it
- Cloud-stored data (analytics, crash reports, feedback) is retained per our Privacy Policy (auto-deleted after retention periods)

### 14.2 Termination by Us

**We may terminate or suspend your access to Sudokan** at any time, with or without notice, for any reason, including:

**Violation of Terms**:
- ✅ Breach of any provision of these Terms
- ✅ Prohibited activities (Section 5)
- ✅ Ad fraud or abuse

**Legal Compliance**:
- ✅ Required by law or court order
- ✅ To comply with legal processes

**Operational Reasons**:
- ✅ Discontinuation of the App (Section 10.6)
- ✅ Service changes or technical issues

**Discretionary**:
- ✅ For any other reason (though we aim to be reasonable)

### 14.3 Effect of Termination

**Upon termination** (by you or us):

**Your obligations**:
- ✅ Immediately stop using the App
- ✅ Uninstall the App from all devices
- ✅ Destroy all copies in your possession

**Our obligations**:
- ✅ Cease providing access to the App (if technically possible)
- ❌ No obligation to refund (app is free)
- ❌ No obligation to provide data export (though Privacy Policy rights remain)

**Survival of Terms**:
- Sections that should survive termination remain in effect:
  - Intellectual Property (Section 4)
  - User Conduct prohibitions (Section 5)
  - Privacy (Section 6 - subject to Privacy Policy)
  - Disclaimer of Warranties (Section 11)
  - Limitation of Liability (Section 12)
  - Indemnification (Section 13)
  - Governing Law (Section 16)
  - Dispute Resolution (Section 17)

### 14.4 No Liability for Termination

**We're not liable for**:
- ❌ Loss of access to the App
- ❌ Loss of game progress or data (back up if important)
- ❌ Inability to use the App in the future
- ❌ Any damages arising from termination

**Termination is without prejudice** to our other rights and remedies.

---

## 15. Modifications to Terms

### 15.1 Right to Modify

**We reserve the right to modify these Terms at any time**, for reasons including:
- Legal compliance (new laws, regulations)
- Feature changes (new app features requiring new terms)
- Clarifications (improving readability, fixing errors)
- Business changes (new monetization models, partnerships)

**Changes are effective immediately** upon posting the updated Terms in the App.

### 15.2 Notice of Material Changes

**For material changes** (e.g., new limitations on use, new fees - if we introduce paid features):
- ✅ We'll update the "Last Updated" date at the top
- ✅ Display an in-app notification (alert dialog) on next app launch
- ✅ Require your acknowledgment to continue using the App

**For non-material changes** (e.g., typo fixes, clarifications):
- ✅ Update "Last Updated" date
- ❌ No in-app notification required

### 15.3 Your Acceptance of Changes

**By continuing to use Sudokan after changes**, you agree to the revised Terms.

**If you disagree with changes**:
- ✅ Stop using the App
- ✅ Uninstall the App
- ✅ You may request data deletion (see Privacy Policy)

**No obligation to continue**: You're free to stop using Sudokan if you don't accept new Terms.

### 15.4 Where to Find Latest Terms

**Current Terms are available**:
- In-app: Settings → Terms and Conditions
- Online: bloombyte.dev/terms (if published)

**We recommend reviewing** these Terms periodically.

---

## 16. Governing Law and Jurisdiction

### 16.1 Governing Law

**These Terms are governed by the laws of Romania**, without regard to conflict of law principles.

**Why Romania**:
- Bloom Byte SRL is a Romanian company
- Our principal place of business is in Romania
- Romania is an EU member state (GDPR compliance built-in)

**Applicable laws include**:
- Romanian Civil Code
- Romanian Consumer Protection Law (Law 296/2004)
- EU regulations (GDPR, Consumer Rights Directive)

### 16.2 Jurisdiction

**For disputes** (see Section 17 for dispute resolution process):

**General Jurisdiction**:
- Courts of Romania have **exclusive jurisdiction** for disputes arising from these Terms
- Specifically: Courts of Bucharest, Romania (our registered office location)

**Consumer Protection**:
- **If you're an EU consumer**, you may also bring claims in your country of residence (EU Consumer Rights Directive)
- **If you're a California consumer**, see Section 19 for specific rights

### 16.3 United Nations Convention on Contracts

**The United Nations Convention on Contracts for the International Sale of Goods (CISG)** does NOT apply to these Terms.

**Why excluded**: CISG is for commercial sales, not consumer software licenses.

### 16.4 Waiver of Jury Trial

**TO THE EXTENT PERMITTED BY LAW, YOU AND BLOOM BYTE SRL WAIVE THE RIGHT TO A JURY TRIAL** for any disputes arising from these Terms.

**Disputes will be resolved by a judge** (or arbitrator, if arbitration is chosen).

**Some jurisdictions may not allow this waiver**. If so, it doesn't apply.

### 16.5 Export Control

**You represent and warrant that**:
- ✅ You're not located in a country subject to U.S. embargo (e.g., North Korea, Syria, Iran, Cuba - subject to change)
- ✅ You're not on U.S. government restricted party lists (e.g., Specially Designated Nationals)

**If you're in a restricted country**, you may not use Sudokan.

**Compliance**: You agree to comply with all export control laws and regulations.

---

## 17. Dispute Resolution

### 17.1 Informal Resolution First

**Before filing a lawsuit or arbitration**, you agree to:

1. **Contact us** at contact@bloombyte.dev with subject "Dispute Resolution"
2. **Describe the dispute** in detail (what happened, what you want)
3. **Allow 30 days** for us to investigate and respond
4. **Negotiate in good faith** to resolve the dispute amicably

**Goal**: Resolve disputes without formal proceedings (saves time and money for both parties).

### 17.2 Arbitration (Optional)

**If informal resolution fails**, you and we may agree to resolve disputes through **binding arbitration** instead of court.

**Arbitration benefits**:
- Faster than court litigation
- Lower costs (no court fees, limited discovery)
- Private (not public record)
- Final (limited appeal rights)

**Arbitration rules**:
- **Arbitration body**: Romanian Court of International Commercial Arbitration (or mutually agreed arbitrator)
- **Location**: Bucharest, Romania (or mutually agreed location)
- **Language**: English or Romanian
- **Costs**: Each party pays its own costs (arbitrator fees split equally)

**Arbitration agreement is optional**: You must explicitly agree to arbitrate (not automatic).

### 17.3 Class Action Waiver

**TO THE EXTENT PERMITTED BY LAW, YOU AGREE TO BRING CLAIMS ONLY IN YOUR INDIVIDUAL CAPACITY**, not as a class action, consolidated action, or representative action.

**No class arbitration**: If you agree to arbitration, you waive class arbitration.

**Why**: Class actions are complex and costly. We offer individual dispute resolution (more practical for a free app).

**Some jurisdictions may not allow this waiver** (e.g., EU, California). If so, it doesn't apply.

### 17.4 Small Claims Court

**You may bring claims in small claims court** (if your jurisdiction has one) instead of arbitration or regular court, if:
- The claim qualifies for small claims (amount and type)
- Small claims court has jurisdiction

**Small claims benefits**: Fast, inexpensive, informal.

### 17.5 Injunctive Relief

**Either party may seek injunctive relief** (court orders to stop harmful conduct) without waiting for arbitration or informal resolution, if:
- Immediate harm is threatened (e.g., intellectual property infringement, security breach)
- Delay would cause irreparable injury

**Example**: If someone is reverse engineering and distributing Sudokan, we may seek an immediate court order to stop them.

### 17.6 EU Online Dispute Resolution

**If you're an EU consumer**, you can use the EU Online Dispute Resolution (ODR) platform:
- **Website**: [ec.europa.eu/consumers/odr](https://ec.europa.eu/consumers/odr)
- **Purpose**: Resolve disputes with online traders

**We're not obligated to participate** in ODR, but we'll consider it if requested.

---

## 18. EU Consumer Rights

**If you're a consumer in the European Union**, you have additional rights under EU law that these Terms cannot override.

### 18.1 Statutory Guarantees

**EU Consumer Rights Directive (2011/83/EU)** provides:

**Right to conformity**:
- ✅ Digital content (Sudokan) must be as described
- ✅ Must be fit for purpose
- ✅ Must have acceptable quality

**If Sudokan fails to conform**:
- You may request repair or update (we'll fix bugs)
- If repair isn't possible, you may request a proportionate price reduction (N/A for free app)
- In serious cases, you may terminate the contract (uninstall and request data deletion)

**Legal guarantee period**: At least **2 years** from delivery (initial download).

### 18.2 No Waiver of Statutory Rights

**These Terms do NOT waive your statutory rights** as an EU consumer, including:
- ✅ Right to conformity (Section 18.1)
- ✅ Right to withdraw (limited for digital content - see Section 18.3)
- ✅ Right to complain to consumer authorities
- ✅ Right to sue in your country of residence

**If these Terms conflict with EU law**, EU law prevails.

### 18.3 Right to Withdraw (Limited)

**EU Consumer Rights Directive** normally gives consumers 14 days to withdraw from distance contracts.

**However**: For digital content, withdrawal rights are waived if:
- ✅ You consented to immediate performance (by using the App)
- ✅ You acknowledged loss of withdrawal right

**By using Sudokan immediately after download**, you waive the 14-day withdrawal right (standard for apps).

**Exceptions**:
- If the App is defective, you have conformity rights (Section 18.1)
- If we violated your rights, you may withdraw and request a refund (N/A for free app)

### 18.4 Unfair Contract Terms

**EU Unfair Contract Terms Directive (93/13/EEC)** prohibits unfair terms in consumer contracts.

**If any term is deemed unfair**:
- The term is unenforceable (but rest of Terms remain)
- We'll replace it with a fair term (or it's deleted)

**Examples of potentially unfair terms** (subject to judicial review):
- Excessive limitation of liability (Section 12) - may be limited for free apps
- Unilateral right to modify terms (Section 15) - we provide notice for material changes
- Broad indemnification (Section 13) - only applies if you violate Terms

**Our position**: These Terms are fair given Sudokan is free and ad-supported. If a court disagrees, we'll comply.

### 18.5 GDPR Rights

**See Privacy Policy** for full GDPR rights (data access, erasure, portability, etc.).

**GDPR rights cannot be waived** by these Terms.

### 18.6 Consumer Dispute Resolution

**If you have a complaint**:
1. Contact us at contact@bloombyte.dev (we'll try to resolve)
2. Contact your national consumer protection authority
3. Use the EU ODR platform: [ec.europa.eu/consumers/odr](https://ec.europa.eu/consumers/odr)
4. File a complaint with Romanian consumer authority (our location)
5. Sue in your country's courts (you can sue in your home country)

**No mandatory arbitration**: EU consumers cannot be forced to arbitrate (Section 17.2 is optional).

---

## 19. California Consumer Rights

**If you're a California resident**, you have additional rights under California law.

### 19.1 CCPA/CPRA Rights

**See Privacy Policy Section 7.3** for full California Consumer Privacy Act (CCPA) and California Privacy Rights Act (CPRA) rights:
- Right to know what data we collect
- Right to delete data
- Right to opt out of "sale" (we don't sell, but you can disable personalized ads)
- Right to correct inaccurate data
- Right to non-discrimination

**CCPA rights cannot be waived** by these Terms.

### 19.2 California Civil Code § 1542 Waiver

**California Civil Code § 1542** provides:

> "A general release does not extend to claims that the creditor or releasing party does not know or suspect to exist in his or her favor at the time of executing the release and that, if known by him or her, would have materially affected his or her settlement with the debtor or released party."

**You expressly waive § 1542** as it relates to releasing us from claims.

**What this means**: If we settle a dispute, you release all claims (even unknown ones). This waiver is common in settlements.

### 19.3 California Consumer Legal Remedies Act (CLRA)

**California CLRA** protects consumers from unfair or deceptive business practices.

**Our compliance**:
- ✅ We disclose all terms clearly (no hidden fees - app is free)
- ✅ We provide accurate descriptions (app does what we say)
- ✅ We honor CCPA rights

**If we violate CLRA**:
- You may demand correction (30-day notice required before lawsuit)
- You may sue for damages, injunction, or attorney fees

**Contact for CLRA complaints**: contact@bloombyte.dev

### 19.4 California Proposition 65 (N/A)

**Prop 65** requires warnings for products containing chemicals known to cause cancer or reproductive harm.

**Sudokan is software (no physical product)**, so Prop 65 doesn't apply.

### 19.5 Arbitration and Class Action Waiver (Limited)

**California law limits class action waivers** in consumer contracts.

**Section 17.3 class action waiver** may not apply to California residents if:
- The dispute involves significant public interest
- The waiver is unconscionable under California law

**If the waiver is unenforceable**, you may bring class actions in California courts.

---

## 20. Severability

### 20.1 Invalid Provisions

**If any provision of these Terms is found to be invalid, illegal, or unenforceable** by a court or arbitrator:

- ✅ That provision is **severed** (removed) from these Terms
- ✅ The **rest of the Terms remain in full effect**
- ✅ The invalid provision is **replaced** with a valid provision that achieves the same intent

**Example**: If a court finds the limitation of liability (Section 12) excessive for EU consumers, that section is modified to comply with EU law, but other sections remain.

### 20.2 Interpretation

**Courts will interpret these Terms** to:
- Give maximum effect to the parties' intent
- Uphold as many provisions as possible
- Avoid rendering any provision meaningless

**No presumption against drafter**: These Terms are not interpreted against us just because we drafted them.

---

## 21. Waiver

### 21.1 No Waiver by Inaction

**Our failure to enforce any provision** of these Terms does NOT constitute a waiver of that provision or our right to enforce it later.

**Example**: If you violate the Terms and we don't immediately terminate your access, that doesn't mean we waive our right to terminate you in the future or enforce the Terms against others.

### 21.2 Waiver Must Be in Writing

**Any waiver of these Terms must be in writing** and signed by an authorized representative of Bloom Byte SRL.

**Verbal waivers are not valid**.

### 21.3 No Implied Waivers

**Waiver of one breach doesn't waive future breaches**.

**Example**: If we allow you to continue using the App after one Terms violation, that doesn't mean we waive our right to terminate you for subsequent violations.

---

## 22. Entire Agreement

### 22.1 Complete Agreement

**These Terms, together with our Privacy Policy, constitute the entire agreement** between you and Bloom Byte SRL regarding Sudokan.

**These Terms supersede**:
- ❌ Any prior agreements (written or verbal)
- ❌ Any prior representations or promises
- ❌ Any course of dealing or trade practice

**No other agreements**: If we've communicated with you via email or support, those communications don't modify these Terms unless we explicitly state they do.

### 22.2 Incorporated Documents

**The following documents are incorporated by reference**:
- ✅ **Privacy Policy** (bloombyte.dev/privacy or in-app)
- ✅ **Apple App Store Terms** (for iOS users)
- ✅ **Google Play Terms** (for Android users)
- ✅ **Third-party service terms** (Firebase, AdMob - see Section 9.1)

**In case of conflict**:
1. These Terms prevail over general statements
2. Specific terms (e.g., Privacy Policy for data issues) prevail over general terms
3. App store terms prevail for store-related issues (refunds, app removal)

### 22.3 No Third-Party Beneficiaries

**These Terms are solely between you and Bloom Byte SRL**.

**No third parties** (e.g., Google, Apple, ad networks) have rights under these Terms, except:
- Apple and Google (as required by app store terms for exculpatory clauses)
- Indemnified parties (Section 13)

---

## 23. Contact Information

### 23.1 How to Contact Us

**For questions, concerns, or requests regarding these Terms**:

**Email**: contact@bloombyte.dev  
**Subject Line Suggestions**:
- "Terms Question: [Your Topic]"
- "Dispute Resolution"
- "DMCA Takedown Request" (Section 4.5)
- "Terms Violation Report" (Section 5.5)

**Response Time**: Within **7-14 business days** (typically faster)

**What to Include**:
- Your name (optional, since app has no accounts)
- Your device model (e.g., "iPhone 14", "Samsung Galaxy S23")
- Your country/region
- Specific question or concern

### 23.2 Company Information

**Legal Name**: Bloom Byte SRL  
**Type**: Societate cu Răspundere Limitată (Limited Liability Company)  
**Registered Office**: Romania  
**Website**: bloombyte.dev  
**Email**: contact@bloombyte.dev

**App Store Presence**:
- **Apple App Store**: Search "Sudokan" or developer "Bloom Byte SRL"
- **Google Play Store**: Search "Sudokan" or developer "Bloom Byte SRL"

### 23.3 Support and Help

**For technical support** (not legal/Terms issues):
- **In-app**: Settings → Help (if available)
- **Email**: contact@bloombyte.dev (subject: "Technical Support")

**Common support requests**:
- Bug reports (describe issue, device model, OS version, steps to reproduce)
- Feature requests (describe desired feature and use case)
- Account/data questions (see Privacy Policy for data requests)

**Response time**: 3-7 business days for support requests.

### 23.4 Feedback and Suggestions

**To provide feedback or suggestions** (general improvement ideas):
- **In-app**: Settings → Send Feedback (anonymous feedback form)
- **Email**: contact@bloombyte.dev (subject: "Feedback")

**Note**: By submitting feedback, you grant us rights to use it (Section 4.3).

### 23.5 Legal Notices

**For legal notices** (e.g., DMCA, cease and desist, subpoenas):
- **Email**: contact@bloombyte.dev (subject: "Legal Notice")
- **Physical mail**: Available upon request (email first)

**Service of process**: Legal documents must be delivered to our registered office in Romania (contact us for address).

### 23.6 Business Inquiries

**For business partnerships, licensing, or other commercial inquiries**:
- **Email**: contact@bloombyte.dev (subject: "Business Inquiry")

**We're open to**:
- White-label licensing (customize Sudokan for your brand)
- B2B partnerships (integrate Sudokan into your platform)
- Sponsorships (brand partnerships for in-app promotions)

---

## 24. Additional Terms

### 24.1 Force Majeure

**We're not liable for failure to perform** due to events beyond our reasonable control, including:
- Acts of God (earthquakes, floods, fires)
- War, terrorism, civil unrest
- Government actions (laws, regulations, embargoes)
- Internet or telecommunications failures
- Pandemics or epidemics
- Strikes or labor disputes (not involving us)

**If force majeure occurs**:
- We'll notify you as soon as reasonably possible
- Our obligations are suspended during the event
- We'll resume performance when the event ends
- If the event lasts >90 days, either party may terminate (no penalty)

### 24.2 Assignment

**You may NOT assign** these Terms or your rights under them to anyone else (e.g., no selling your "right to use Sudokan").

**We may assign** these Terms to:
- Affiliates (companies we own or control)
- Successors (if we're acquired or merge)
- Third parties (with notice to you)

**Upon assignment**, the assignee assumes our rights and obligations.

### 24.3 Notices

**Notices to you**:
- In-app notifications (alerts, banners)
- Email (if you provide it via support requests - we don't collect emails by default)
- Posted in the App (Settings → Announcements, if we add this feature)

**Notices to us**:
- Email: contact@bloombyte.dev

**Effective date**: Notices are effective upon sending (email) or posting (in-app).

### 24.4 Language

**These Terms are written in English**. If we provide translations:
- Translations are for convenience only
- English version is authoritative
- In case of conflict, English prevails

### 24.5 Headings

**Section headings** (e.g., "Disclaimer of Warranties") are for convenience only and do NOT affect interpretation.

---

## 25. Acknowledgment and Acceptance

### 25.1 You Acknowledge

By using Sudokan, you acknowledge that you have:

- ✅ **Read** these Terms and Conditions in their entirety
- ✅ **Understood** the provisions, especially:
  - Disclaimer of Warranties (Section 11)
  - Limitation of Liability (Section 12)
  - Indemnification (Section 13)
  - Governing Law (Section 16)
- ✅ **Had the opportunity** to seek legal advice (if desired)
- ✅ **Agreed** to be bound by these Terms

### 25.2 Effective Date

**These Terms are effective as of**: November 9, 2025

**Your acceptance date**: The date you first download, install, or use Sudokan (whichever is earliest).

### 25.3 Version Control

**Current Version**: 1.0  
**Last Updated**: November 9, 2025

**Version history** (future updates will be listed here):
- **v1.0** (November 9, 2025): Initial Terms and Conditions for Sudokan launch

### 25.4 Questions?

**If you have questions about these Terms**, please contact us at contact@bloombyte.dev before using the App.

**We're happy to clarify** any provisions or address concerns.

---

## Summary

Thank you for reading Sudokan's Terms and Conditions! Here's a quick recap:

### What You Can Do
- ✅ **Use Sudokan** for free on your personal devices (phone, tablet)
- ✅ **Play as much as you want** (no time limits, no usage caps)
- ✅ **Control your privacy** (opt in/out of analytics, crash reporting, personalized ads)
- ✅ **Provide feedback** (help us improve the app)

### What You Can't Do
- ❌ **Reverse engineer** or copy the app
- ❌ **Use commercially** (no selling puzzle solutions or using in tournaments with fees)
- ❌ **Exploit or hack** (no bots, no ad fraud)
- ❌ **Violate laws** or third-party rights

### Our Promises
- ✅ **Free app** (no hidden fees, no required purchases)
- ✅ **Privacy-focused** (minimal data collection, consent-based)
- ✅ **Continuous improvement** (bug fixes, updates, new features)
- ✅ **Transparency** (clear Terms, Privacy Policy, support)

### Key Legal Points
- ⚖️ **"AS IS" basis** (no warranties, use at your own risk)
- ⚖️ **Limited liability** (max $100 USD, no indirect damages)
- ⚖️ **Romanian law** (EU consumer protections apply)
- ⚖️ **13+ age requirement** (COPPA/GDPR compliance)

### Your Rights
- 🛡️ **EU consumers**: 2-year guarantee, conformity rights, no waiver of statutory rights
- 🛡️ **California consumers**: CCPA rights (access, delete, opt-out, correct)
- 🛡️ **Everyone**: Informal dispute resolution, small claims court option

### Questions or Concerns?
- 📧 **Email**: contact@bloombyte.dev
- 🌐 **Website**: bloombyte.dev
- 📱 **In-app**: Settings → Terms and Conditions

**By continuing to use Sudokan, you agree to these Terms. Enjoy the game! 🎉**

---

**END OF TERMS AND CONDITIONS**

---

**Document Information**:
- **Version**: 1.0
- **Last Updated**: November 9, 2025
- **Effective Date**: November 9, 2025
- **Developer**: Bloom Byte SRL
- **Contact**: contact@bloombyte.dev
- **Website**: bloombyte.dev
- **App**: Sudokan (iOS and Android)
- **Governing Law**: Romania
- **Language**: English (authoritative version)
